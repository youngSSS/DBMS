...
pagenum_t file_alloc_page(int table_id)
{
    ...
    if (header_page == NULL)
        printf("file_alloc_page in file.c\n");
    if (free_page == NULL)
        printf("file_alloc_page in file.c\n");
...
void file_free_page(int table_id, pagenum_t pagenum)
{
    ...
    if (header_page == NULL)
        printf("file_free_page in file.c\n");
    if (target_page == NULL)
        printf("file_free_page in file.c\n");
...
void file_write_page(int table_id, pagenum_t pagenum, const page_t *src)
{
    ...
    if (result == -1)
        printf("write_to_file fault in file.c\n");
}
...
page_t *make_free_pages(int table_id, page_t *header_page)
{
    ...
    free_page = (page_t *)malloc(sizeof(page_t));
    if (free_page == NULL)
        printf("make_free_pages in disk.cpp\n");
