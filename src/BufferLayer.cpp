...
int buf_close(int table_id) {
	// Write down all pages in table <table_id> from buffer to disk.
	if (Buffer != nullptr) buf_flush(table_id);
...
int buf_init_db(int num_buf) {
	...
	for (int i = 0; i < num_buf; i++) {
		...
		if (pthread_mutex_init(&Buffer[i].page_latch, nullptr) != 0) return 1;
	}
	...
	// Set mutex
	if (pthread_mutex_init(&Buffer_Latch, nullptr) != 0) return 1;

	return 0;
}

int buf_shutdown_db() {
	// Case: Buffer does not exist
	if (Buffer == nullptr) return 0;

	// Case: Buffer exists
	// Flush all data in the buffer to disk and close the file
	for (int i = 1; i <= 10; i++) {
		buf_flush(i);
		file_close(i);
	}
...
// Write every dirty page of table <table_id> from buffer to disk.
void buf_flush(int table_id) {
	// WAL protocol
	write_log(0, 0);
	write_log(1, 0);

	for (auto iter = Buffer_Header.hash_table[table_id].begin(); iter != Buffer_Header.hash_table[table_id].end();
		 iter++) {
...
void LRU_linking(int framenum) {
	...
	if (Buffer_Header.LRU_head == -1) {
		...
		if (Buffer_Header.LRU_head == framenum) return;
...
framenum_t LRU_policy() {
	...
	while (true) {
		...
		else framenum = Buffer[framenum].prev_of_LRU;
	}
...
void mutex_buf_write(int table_id, pagenum_t pagenum, const page_t* src, int page_latch_flag) {
	...
	/* Case: Thread failed to catch page latch */
	if (framenum == -1) printf("mutex_buf_write_ERROR\n");
