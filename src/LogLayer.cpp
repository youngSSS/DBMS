	...
	while (true) {
		...
		if (type == 1) {
			...
			if (page->p.page_LSN < update_log.lsn) {
				...
				fprintf(Log_FP,
					"LSN %d [UPDATE] Transaction id %d redo apply\n",
					update_log.lsn + LOG_U_SIZE,
					g_log.trx_id);
			}

			else
				fprintf(Log_FP,
					"LSN %d [CONSIDER-REDO] Transaction id %d\n",
					update_log.lsn + LOG_U_SIZE,
					update_log.trx_id);
			...
			if (page->p.page_LSN < compensate_log.lsn) {
				...
				fprintf(Log_FP,
					"LSN %d [CLR] next undo lsn %d\n",
					compensate_log.lsn + LOG_C_SIZE,
					compensate_log.next_undo_LSN);
//                printf("RECOVERY :: [REDO] trxid: %d, O - %s, N - %s\n", compensate_log.trx_id, compensate_log.old_img, compensate_log.new_img);
			...
			if (type == 0) fprintf(Log_FP, "LSN %d [BEGIN] Transaction id %d\n", g_log.lsn + LOG_G_SIZE, g_log.trx_id);
				...
				fprintf(Log_FP,
					"LSN %d [COMMIT] Transaction id %d\n",
					g_log.lsn + LOG_G_SIZE,
					g_log.trx_id);
...
int undo_pass(int flag, int log_num, int* start_offset) {
	...
	while (Undo_List.size() != 0) {
		...
		if (undo_flag == 1) {
			...
			fprintf(Log_FP,
				"LSN %d [UPDATE] Transaction id %d undo apply\n",
				update_log.lsn + LOG_U_SIZE,
				update_log.trx_id);
