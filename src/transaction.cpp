...
int init_trx()
{
    if (pthread_mutex_init(&Trx_Global_Latch, NULL) != 0)
        return 1;
    if (pthread_mutex_init(&Trx_Table_Latch, NULL) != 0)
        return 1;
    if (pthread_mutex_init(&Trx_Abort_Latch, NULL) != 0)
        return 1;
}
...
void trx_abort(int trx_id)
{
    ...
    while (undo_log_list.size() != 0)
    {
        ...
        if (undo_log_list.size() == 0)
            next_undo_LSN = -1;
        else
            next_undo_LSN = undo_log_list.back().lsn;
