...
string Filename[11];
uint8_t Is_open[11]; // File descriptors
...
int file_open(char* pathname) {
	...
	if (Is_open[table_id] > 0) return table_id;

	// Case: File name format is wrong
	if (table_id == -2) return -2;

	fd = open(pathname, O_RDWR | O_CREAT, S_IRWXU);
	if (fd == -1) return -1; // Case: Fail to open the file

	Filename[table_id] = temp_pathname;
	Is_open[table_id] = fd;
	...
	if (file_size == 0) {
		...
		if (header_page == nullptr) return -3;
...
pagenum_t file_alloc_page(int table_id) {
	...
	if (header_page == nullptr) printf("file_alloc_page in DiskLayer.cpp\n");
	if (free_page == nullptr) printf("file_alloc_page in DiskLayer.cpp\n");
...
void file_free_page(int table_id, pagenum_t pagenum) {
	...
	if (header_page == nullptr) printf("file_free_page in DiskLayer.cpp\n");
	if (target_page == nullptr) printf("file_free_page in DiskLayer.cpp\n");
...
void file_read_page(int table_id, pagenum_t pagenum, page_t* dest) {
	int fd = Is_open[table_id];
	int result = pread(fd, dest, PAGE_SIZE, pagenum * PAGE_SIZE);

	if (result == -1) printf("read_from_file fault in DiskLayer.cpp\n");
}

// Write an in-memory page(src) to the on-disk page
void file_write_page(int table_id, pagenum_t pagenum, const page_t* src) {
	int fd = Is_open[table_id];
	int result = pwrite(fd, src, PAGE_SIZE, pagenum * PAGE_SIZE);

	if (fsync(fd) != 0) printf("fsync fault\n");
	if (result == -1) printf("write_to_file fault in DiskLayer.cpp\n");
}
...
page_t* make_free_pages(int table_id, page_t* header_page) {
	...
	free_page = (page_t*)malloc(sizeof(page_t));
	if (free_page == nullptr) printf("make_free_pages in DiskLayer.cpp\n");
...
int get_table_id(string pathname) {
	for (int i = 1; i <= 10; i++) {
		if (pathname == "DATA" + to_string(i)) return i;
	}
	return -2;
}
